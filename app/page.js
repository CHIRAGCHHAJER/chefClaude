...
import {
  ...
  handleMouseDown,
  handleMouseUp,
} from './components/InteractionsUI-UX/interactionFunction.js'
import Form from './components/Form/page.js'
import Recipe from './components/Recipe/page.js'
  ...
  async function getRecipe() {
    ...
    try {
      const response = await fetch(
        `/api/getRecipe?ingredients=${ingredientString}`
      )
      if(!response.ok){
        setRecipe("") 
        setError(`Sorry we are having a server side error , Please try again .`)
        setSearching(false)
        return 
      }
      ...
      console.error(`We are having issue with connecting to the Server\nERROR : `, error)
      setRecipe("")
      setError(`We are having issue with connecting to the Server.`)
      setSearching(false)
    }
  }
  ...
  return (
    <>
      <div className='div1  bg-[rgba(221,217,199,0.7)] flex flex-col  h-[95vh] min-h-[710px] w-[90vw] min-w-[490px] border-[3px] border-solid border-[rgba(221,217,199,0.9)] rounded-[3px] shadow-inner py-[1px] px-[1px] -px-[1px] overflow-y-auto'>
        <header className='header bg-[rgba(213,201,155,0.9)] flex flex-row justify-center items-center gap-[4px] shadow-lg rounded-[3px] min-h-[70px] w-[100%]'>
          <Image
            height={70}
            width={70}
            className='appLogo h-[70px] w-[70px]'
            src='/logo/cooking.png'
            alt='Logo'
          />
          <h1 className=' font-semibold text-[35px] text-[rgba(125,120,95,1)]'>
            Chef Claude
          </h1>
        </header>
        ...
        {ingredients && ingredients.length > 0 ? (
          <h1 className='text-[20px]  text-[rgba(125,120,95,1)] mx-[2px] font-[700] '>
            Ingredients on Hand :{' '}
          </h1>
        ) : null}
        ...
        <ul className=' list-disc list-inside marker:text-[#47463f] '>
          {ingredients.map((ingredient, index) => {
            return (
              <li
                key={index}
                className='text-[18px] text-[rgba(125,120,95,1)] font-sans font-[520] '
              >
                {ingredient}
              </li>
            )
        ...
        {ingredients && ingredients.length > 0 && (
          <div className='flex flex-col justify-center min-h-[80px] w-[98%]'>
            <h1 className='text-[18px] text-[rgba(125,120,95,1)] font-[500] mx-[2px]'>
              Ready for a recipe :{' '}
              <span className='text-[16px]'>
                Get the recipe for the list of ingredients we have .
              </span>
            </h1>
            <div className='flex flex-row justify-center'>
              <button
                ...
                className='bg-[rgba(213,201,155,0.9)] text-[rgba(125,120,95,1)] font-[500] min-w-[120px] w-[120px] h-[35px] border-[2px] border-solid border-[rgba(125,120,95,1)] shadow-md  rounded-[7px]'
              >
                Get a Recipe
              </button>
            </div>
        ...
        {searching && (
          <div className='text-[16px] text-[rgba(125,120,95,1)] font-[500] mx-[2px]'>
            Searching for the best recipes for you...
            😋🔥🍳🍝🥗🌶️🧀🍅🌿🍞🥔🍋🥑🔥😋
          </div>
        )}
        {error !== "" && (
          <div className='flex flex-row justify-center text-[16px] text-red-400 font-[800] mx-[2px]'>
            {error}
          </div>
        )}
